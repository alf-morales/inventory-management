<template>
  <div class="orders">
    ...
    <div v-else>
      ...
      <!-- Submitted restocking orders come from a separate backend list and are dated 2026 with
           a status that matches none of the four global filters, so this section deliberately
           ignores the global FilterBar (precedent: Spending.vue's monthly chart also ignores it). -->
          ...
          <table class="orders-table">
            ...
            <tbody>
              <tr v-for="order in submittedOrders" :key="order.id">
                ...
                <td class="col-lead">{{ t('orders.leadTimeDays', { days: order.lead_time_days }) }}</td>
  ...
  setup() {
    ...
    const loadOrders = async () => {
      try {
        ...
        submittedOrders.value = fetchedSubmitted.sort((a, b) => new Date(b.order_date) - new Date(a.order_date))
      } catch (err) {
